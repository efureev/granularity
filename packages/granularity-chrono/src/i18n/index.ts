 ...
 *
 * Ключи лежат под собственным неймспейсом `grChrono`, а не под `gr` ядра:
 * компонент спрашивает `grChrono.calendar.previousMonth`. Блок объявлен
 * константой `GR_CHRONO_I18N_BLOCK` — приложение регистрирует именно его.
 * Английский текст продублирован в компоненте
