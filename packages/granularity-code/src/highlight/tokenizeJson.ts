...
import type { GrCodeRole, GrCodeToken } from './palette'

export type { GrCodeRole, GrCodeToken }
...
function readString(source: string, start: number): number {
  ...
  while (index < source.length) {
    // `noUncheckedIndexedAccess` строже, чем в ядре: индекс в границах цикла
    // всегда попадает в строку, и `!` тут констатация, а не допущение.
...
function readNumber(source: string, start: number): number {
  ...
  if (source[index] === '-')
    index += 1
  while (index < source.length && /[\d.e+-]/i.test(source[index]!)) index += 1
...
export function tokenizeJson(source: string): GrCodeToken[] {
  ...
  const flushPlain = (until: number): void => {
    if (until > plainStart)
      tokens.push({ text: source.slice(plainStart, until), role: 'plain' })
  }
  ...
  while (index < source.length) {
    ...
    if (char === '"') {
      ...
      const role: GrCodeRole = nextMeaningful(source, end) === ':' ? 'key' : 'string'
      tokens.push({ text: source.slice(index, end), role })
      index = end
      plainStart = index
      continue
    }
    ...
    if (PUNCTUATION.has(char)) {
      flushPlain(index)
      tokens.push({ text: char, role: 'punctuation' })
      index += 1
      plainStart = index
      continue
    }
    ...
    if (char === '-' || (char >= '0' && char <= '9')) {
      ...
      tokens.push({ text: source.slice(index, end), role: 'number' })
      index = end
      plainStart = index
      continue
    }
    ...
    if (literal) {
      flushPlain(index)
      tokens.push({ text: literal, role: 'literal' })
