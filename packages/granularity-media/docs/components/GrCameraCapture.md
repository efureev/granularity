...
## Рамка принимает соотношение камеры

Без `aspectRatio` рамка показывает поток как есть: камеры отдают то 4:3, то
16:9, и зашитое число выдало бы обрезанный кадр за то, что видит камера.
Соотношение известно не сразу — до `loadedmetadata` размеры кадра нули, поэтому
до первого кадра рамка держит 4:3, чтобы её место в раскладке было видно.

Заданный `aspectRatio` сильнее: приложение знает, куда встанет снимок. Тогда
лишнее срезается по центру — ровно так же, как это делает `object-fit: cover` в
превью, и снимок совпадает с тем, что пользователь видел.
