...
| Откуда виджет берёт данные | [`granularity-datasource`](../../../granularity-datasource/README.md) |
| Чем виджет рисует содержимое | [`granularity-charts`](../../../granularity-charts/docs/components.md) / [`granularity-chrono`](../../../granularity-chrono/docs/components.md) |
...
Подробности замера — [`./GrDashboardItem.md`](./GrDashboardItem.md).

## `itemTransferOut` — виджет уехал к соседям

Два дашборда на странице обмениваются виджетами перетаскиванием: жест за ручку,
ушедший за край своей сетки в чужую, становится переносом между ними.
...
приехал. Разбор — [`../model.md`](../model.md).
...
- **виртуализации сетки** — виджетов на дашборде десятки, а не тысячи, и цена
  сложности не окупается;
- **вложенных дашбордов** — сетка в сетке даёт неразрешимый вопрос, чей жест
  обрабатывать;
- **своей карточки вместо `GrCard`** — копия разошлась бы с карточками
  остального приложения;
- **конструктора отчётов** с полями и агрегатами — это продукт, а не компонент.
