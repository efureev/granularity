...
/**
 * Покомпонентные переменные сетки виджетов потребитель видит как публичный API
 * темизации: он их переопределяет. Без реестра пакет не видит их вовсе —
 * переименование не роняет ни одного теста.
 *
