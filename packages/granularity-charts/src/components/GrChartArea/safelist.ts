 ...
 *
 * Своих классов у площади нет — она рисуется атрибутами SVG, — но рама живёт в
 * общем `.ts`-хелпере, который бандлер уносит в `dist/chunks/`. Пресет
 * сканирует только `dist/components/GrChartArea/**`, поэтому без этой строки
 * график собирается зелёным и приезжает к потребителю без цветов и без
 * фокус-кольца.
 */
export const grChartAreaSafelist: string[] = [...chartFrameSafelist]
