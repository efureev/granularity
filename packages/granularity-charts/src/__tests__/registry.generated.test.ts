...
describe('реестры компонентов', () => {
  ...
  it('в реестре провайдера ровно публичные компоненты', () => {
    expect(Object.keys(granularityChartsComponentConfigs).sort()).toEqual([...publicComponents].sort())
  })
  ...
  it('список имён совпадает с реестром провайдера', () => {
    ...
    // они не имеют права.
    expect([...GRANULARITY_CHARTS_COMPONENTS].sort()).toEqual([...publicComponents].sort())
  })
