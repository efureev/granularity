...
Без 3D, без географии (это карта) и без аннотаций. Приближение по абсциссе есть — протяжкой и колесом, проп `zoom` у
`GrChartLine` и `GrChartArea`; **клавиатурного пути к окну нет**, поэтому окно выведено наружу как `v-model:xWindow` —
доступное управление им ставит приложение. Порог SVG — около 2 000 точек на серию; canvas-путь за тем же API придёт
позже.
