...
`v-model:x-window` не обязателен — без привязки график приближается сам. Но
привязать его стоит: **клавиатурного пути к окну у графика нет**, и доступное
управление им — кнопку сброса, поля диапазона — ставит приложение. Разбор —
[`../a11y.md`](../a11y.md), раздел про приближение.
