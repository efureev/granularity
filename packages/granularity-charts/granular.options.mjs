 ...
 * компоненты транзитивного донора, а их проверяет собственный `doctor` ядра.
 */
// Импорт из `dist` здесь не оплошность, а суть проверки: доктор сверяет граф
// того, что отгружается, а не исходников.
// eslint-disable-next-line antfu/no-import-dist
import granularityChartsProvider from './dist/granular-provider.js'

export default {
  providers: [granularityChartsProvider],
  components: [{ provider: '@feugene/granularity-charts', names: 'all' }],
}
