 ...
 * `<GrConfigProvider size="sm">` масштабировал `GrInput` и не трогал стоящий
 * рядом `GrTextarea`, из-за чего форма визуально рассыпалась.
 *
 ...
 * гейтом; каждый требует отдельного решения по внешнему виду, поэтому чинить их
 * заодно с общим переводом на шкалу нельзя:
 *
