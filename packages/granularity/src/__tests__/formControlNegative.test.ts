...
describe('readonly: ни одна клавиша не меняет значение', () => {
  ...
  /*
   * Порог поднят по той же причине, что у safelist-гейта: тест монтирует
   * каждый форм-контрол пакета и прогоняет по нему клавиатуру, и под
   * параллельной нагрузкой полного набора перебирал дефолтные 5 с.
   */
