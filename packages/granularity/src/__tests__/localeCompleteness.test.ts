...
defineLocaleCompletenessGate({
  block: 'gr',
  // Ядро строит часть ключей шаблоном (`` t(`gr.form.${kind}`) ``), и статически
  // такой ключ не восстановить: 183 найденных против 241 объявленного.
  keyParity: false,
  // Формы множественного числа: `one`/`few`/`many`/`other` — не ключи словаря.
  pluralForms: true,
})
