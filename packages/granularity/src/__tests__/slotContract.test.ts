...
const DOMAIN_SLOTS: Record<string, string[]> = {
  ...
  'center': ['GrNavbar'],
  // Своя формулировка счётчика символов. Не `description` и не `label`: те
  // описывают поле целиком и приходят от `GrFormField`, а здесь подпись про
  // одну величину — длину значения — и живёт она в ряду счётчиков.
  'count': ['GrTextarea'],
  ...
  'step': ['GrSteps'],
  'item': ['GrBottomNav', 'GrBreadcrumbs', 'GrCommandPalette', 'GrList', 'GrScrollSpy', 'GrSortableList', 'GrTimeline', 'GrTransfer'],
