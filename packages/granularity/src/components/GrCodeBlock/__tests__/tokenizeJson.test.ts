...
import { tokenizeJson, type GrCodeTokenKind } from '../tokenizeJson'

/** Роль по тексту токена — так тесты читаются, а не считают индексы. */
function kindOf(source: string, text: string): GrCodeTokenKind | undefined {
  return tokenizeJson(source).find(token => token.text === text)?.kind
}
...
describe('tokenizeJson', () => {
  ...
  it('пробелы и переносы остаются plain', () => {
    ...
    expect(tokens.some(token => token.kind === 'plain' && token.text.includes('\n'))).toBe(true)
  })
  ...
  it('литерал не выкусывается из середины слова', () => {
    ...
    expect(tokenizeJson(source).some(token => token.kind === 'literal')).toBe(false)
  })
})
