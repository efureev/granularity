...
import { createDsTreeDataAdapter } from './dsTreeDataAdapter'
import { createDsTreeInteractionContext } from './dsTreeInteractionContext'
import { createDsTreeStore } from './dsTreeStore'
import type {
  DsTreeBranchLineColor,
  DsTreeNodeClass,
  DsTreeProps,
  DsTreeVisibleRow,
  DsTreeVisibleTreeRow,
} from './dsTreeProps'
...
const props = withDefaults(defineProps<DsTreeProps<T>>(), {
  props: () => ({
    children: 'children',
    label: 'label',
  }),
...
const emit = defineEmits<{
  (event: 'nodeClick', data: T, node: DsTreeNode<T>): void
  (event: 'nodeExpand', data: T, node: DsTreeNode<T>): void
  (event: 'nodeCollapse', data: T, node: DsTreeNode<T>): void
  (event: 'nodeDrop', draggingNode: DsTreeNode<T>, dropNode: DsTreeNode<T>, dropType: DsTreeNodeDropType): void
}>()

defineSlots<{
  default?: (props: { node: DsTreeNode<T>; data: T }) => any
}>()

const dataAdapter = createDsTreeDataAdapter(props)
const treeStore = props.internalStore ?? createDsTreeStore({
  adapter: dataAdapter,
  data: () => props.data,
  defaultExpandedKeys: () => props.defaultExpandedKeys,
  filterNodeMethod: () => props.filterNodeMethod,
})
const interactionContext = props.internalInteractionContext ?? createDsTreeInteractionContext(props, {
...
const treeProps = props as Readonly<DsTreeProps<T>>
...
const visibleRows = computed<DsTreeVisibleTreeRow<T>[]>(() => {
  ...
  const walk = (nodes: DsTreeNode<T>[]): DsTreeVisibleTreeRow<T>[] => {
    const rows: DsTreeVisibleTreeRow<T>[] = []
...
function onRowClick(node: DsTreeNode<T>) {
  treeStore.setCurrentKey(node.key)
  interactionContext.emitNodeClick(node.data, node)
}

function onRowMouseEnter(node: DsTreeNode<T>) {
  hoveredKey.value = node.key
}

function onRowMouseLeave(node: DsTreeNode<T>) {
...
function shouldShowDragHandle(node: DsTreeNode<T>): boolean {
  return canDrag(node) && hoveredKey.value === node.key
}

function resolveDropType(evt: DragEvent, el: HTMLElement): DsTreeNodeDropType {
...
function onDragStart(evt: DragEvent, node: DsTreeNode<T>) {
  if (!canDrag(node)) {
    evt.preventDefault()
    return
  }
...
function onDragOver(evt: DragEvent, node: DsTreeNode<T>, rowEl: HTMLElement) {
...
function onDrop(evt: DragEvent, node: DsTreeNode<T>) {
  const drag = interactionContext.draggingNode.value
...
function resolveNodeClass(classValue: DsTreeNodeClass<T> | undefined, row: DsTreeVisibleRow<T>) {
  if (!classValue)
    return undefined
...
function resolveBranchLineColorValue(color: DsTreeBranchLineColor<T> | undefined, node: DsTreeNode<T>) {
  if (!color)
    return undefined
...
function isBranchLineActive(node: DsTreeNode<T>) {
...
function resolveBranchLineColor(node: DsTreeNode<T>) {
...
function resolveChildrenWrapStyle(row: DsTreeVisibleTreeRow<T>) {
