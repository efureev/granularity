...
describe('hslaToCss и normalizeHsla', () => {
  ...
  /**
   * Приведение стоит на пути **каждого** коммита, поэтому округли оно каналы —
   * и компонент переписывал бы чужой цвет без единого движения бегунка: хватало
   * выбрать пресет или вставить hex. Сетка целых HSL мельче сетки hex, и здесь
   * перечислены типовые брендовые значения, которые на неё не садятся.
   */
  it('приведение не сажает цвет на целую сетку HSL', () => {
    const brand = [
      '#3b82f6',
      '#10b981',
      '#ef4444',
    ...
    for (const hex of brand) {
      expect(formatHexColor(normalizeHsla(parseHexColor(hex)!))).toBe(hex)
    }
  })
})
