...
function dispatch(doc: Document, event: Event, capture: boolean): void {
  ...
  for (const el of [...elements]) {
    ...
    if (!enabled || !handler)
      continue
    if (state.options.capture !== capture)
      continue
    if (!events.includes(event.type as NormalizedOptions['events'][number]))
      continue
    ...
    if (!el.isConnected)
      continue
    if (isEventInside(el, event))
      continue

    const excluded = resolveExclude(state.doc, exclude)
    if (excluded.some(item => isEventInside(item, event)))
      continue
