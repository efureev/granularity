...
| `--gr-code-block-number` | только хук | var(--gr-info-text) | Цвет числа. |
...
| Токен | Откуда | По умолчанию | Назначение |
| --- | --- | --- | --- |
| `--gr-file-preview-bg` | только хук | var(--gr-muted) | Фон плитки. Виден у заглушки целиком, а у картинки — пока она едет и по краям при несовпадении пропорций. |
