...
| `--gr-info-light` | `#dbeafe` | `#1e3a8a` | Облегчённая info-подложка для спокойных информационных блоков. |
...
| `--gr-timeline-columns` | CSS компонента | `auto 1fr`, а по раскладкам — с колонкой времени либо симметричная | Сетка колонок ленты. Задаётся собственным CSS компонента по `layout`; строка события и заголовок группы делят её, чтобы ось у них совпадала. |
