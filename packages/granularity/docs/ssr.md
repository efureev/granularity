...
будучи открытым: телепорт включается только после монтирования, а до него
поддерева слоя в разметке нет. Практический вывод: на первый экран и в SEO содержимое модалки не
...
5. DOM-идентификаторы — только `useId()`. Ни `instance.uid` (сквозной счётчик
   приложения: на сервере растёт между запросами, на клиенте стартует с нуля),
   ни `Math.random()`. Расхождение придёт молча — через `id`, `aria-controls`,
   `aria-labelledby` и `name` скрытых полей.
6. Значение, зависящее от среды (`navigator`, ширина окна, тема ОС), в первом
   рендере использовать нельзя: заводите `ref` с серверным значением и
   уточняйте его в `onMounted` — образец в `GrCommandPalette` (`isApple`).
...
- `apps/playground-ssr` — SSR-стенд: живой dev-сервер, тесты серверного рендера
  и гидрации, воспроизведение дефекта §60. Три страницы-улики: `App.vue`
  (изоморфные), `TeleportPage.vue` (телепорт), `RiskyPage.vue` (браузерный API
  в setup, `navigator`, авто-id). Гоняется в CI джобой `test-playground-ssr`.
