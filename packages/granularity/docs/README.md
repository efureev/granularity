...
- Если страница длинная и нужно показать, до какого раздела дочитали: откройте [`scroll-spy.md`](./scroll-spy.md).
- Если дерево нужно со своей разметкой или своими ролями: откройте [`tree-model.md`](./tree-model.md).
...
- [`styling.md`](./styling.md) — `styles.css`, `tokens`, `base`, темы, порядок импортов и почему
...
- [`scroll-spy.md`](./scroll-spy.md) — `useScrollSpy`: активный раздел по прокрутке, линия активации, закреп подсветки на время перехода.
- [`tree-model.md`](./tree-model.md) — `useTree`: модель дерева без разметки — раскрытие, отметки с наследованием, фильтр, ленивая подгрузка, плоский список видимых строк.
