...
Ещё два пакета семейства компонентов не добавляют:
...
- **`@feugene/extra-granularity`** `0.2.0` — составные компоненты с префиксом
  `Xg` поверх примитивов ядра (сейчас один — `XgQuickForm`). Служит
  интеграционной проверкой связки «пакет + пресет»; ни одним приложением этого
