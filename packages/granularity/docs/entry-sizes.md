...
> Сгенерировано `yarn sizes:docs` по собранному `dist` пакета `@feugene/granularity` 0.45.0.
...
**Складывать эти числа нельзя.** Общий чанк посчитан в каждой строке заново, а платится один раз: сумма 5 самых тяжёлых строк даёт 384.4 kB, а вместе они весят 173.8 kB. Вес набора считается объединением — так его и считает `yarn sizes`.
...
| `GrResponseErrorBanner` | 26.2 kB | 14 | 4 % |
| `GrCarousel` | 26.0 kB | 17 | 4 % |
| `GrSortableList` | 24.2 kB | 16 | 4 % |
| `GrList` | 22.5 kB | 16 | 4 % |
| `GrNumberInput` | 20.6 kB | 18 | 3 % |
