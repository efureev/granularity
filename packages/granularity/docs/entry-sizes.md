...
> Сгенерировано `yarn sizes:docs` по собранному `dist` пакета `@feugene/granularity` 0.46.0.
...
**Складывать эти числа нельзя.** Общий чанк посчитан в каждой строке заново, а платится один раз: сумма 5 самых тяжёлых строк даёт 384.9 kB, а вместе они весят 174.3 kB. Вес набора считается объединением — так его и считает `yarn sizes`.
...
| `GrTreeSelect` | 73.2 kB | 43 | 12 % |
| `GrSelect` | 67.8 kB | 41 | 11 % |
| `GrColorPicker` | 64.6 kB | 35 | 11 % |
| `GrJsonViewer` | 61.0 kB | 36 | 10 % |
