...
ответа. Держите ссылку стабильной: инлайн-литерал пересоздаётся каждым
ререндером и сбрасывал бы remote-результаты (та же конвенция идентичности, что
у `useVirtualList.source`).
