...
- **список приходит с сервера** — `source` и `debounce` вместе с состояниями «идёт поиск» и «ничего не найдено»;
- **нужен минимум символов** — `minQueryLength` не даёт уйти на сервер за первой же буквой;
- **значений несколько** — `multiple` показывает выбранное чипами, каждый снимается с клавиатуры;
- **значения может не быть в справочнике** — `allowCustomValue` разрешает своё.

## Когда взять другое

| Нужно | Берите |
| --- | --- |
| Вариантов 5–50 и их просматривают, а не ищут | [`GrSelect`](./GrSelect.md) |
| Варианты — дерево с уровнями | [`GrTreeSelect`](./GrTreeSelect.md) |
| Ищут команду приложения, а не значение поля | [`GrCommandPalette`](./GrCommandPalette.md) |
| Справочника нет вовсе, вводят свои строки | [`GrInputTag`](./GrInputTag.md) |
| Вариантов 2–5 и они видны сразу | [`GrSegmented`](./GrSegmented.md) |
...
<GrAutocomplete
  v-model="city"
  :options="[
    { label: 'Европа', options: [{ value: 'lis', label: 'Лиссабон' }] },
    { label: 'Азия', options: [{ value: 'tyo', label: 'Токио' }] },
    { value: 'remote', label: 'Удалённо' },
  ]"
/>
```

Форма данных та же, что у [`GrSelect`](./GrSelect.md): два соседних контрола, у
которых списки различаются только способом ввода, обязаны принимать данные
одинаково. Группы и одиночные опции смешиваются в одном массиве.

**Фильтр прячет группу целиком**, если в ней ничего не совпало. Заголовок над
пустотой читается как сбой, а не как «здесь ничего нет».

**Клавиатура ходит только по опциям.** Заголовок группы стрелками не выбирается
и в набор для диктора не входит: прямыми потомками `role="listbox"` обязаны быть
опции, поэтому заголовок лежит **внутри** своей группы и даёт ей имя через
`aria-labelledby`. При виртуализации окно может начаться ниже заголовка — тогда
имя группы уходит в `aria-label`, и группа всё равно остаётся названной.

Одинаковое значение в разных группах — не ошибка: ключ строки собирается с
индексом группы, поэтому две такие опции не схлопываются в один узел, а
наведение мышью подсвечивает ту, на которую навели.

Группы приходят и от сервера: `fetchOptions` вправе вернуть тот же массив с
группами, и подпись состава считается по их содержимому — смена опции внутри
группы не проходит незамеченной.
...
`dropdownMaxHeight` — потолок **списка**, а не панели. Саму панель ограничивает
место, оставшееся до края вьюпорта: слой сообщает замер в
`--gr-floating-available-height`, и на коротком экране сжимается именно список —
поле ввода и строка статуса остаются на виду, а полоса прокрутки одна. Механизм общий на списочные
панели пакета и описан в [`../overlays.md`](../overlays.md).

## Виртуализация

`virtual` оставляет в DOM только окно вокруг вьюпорта — его высоту задаёт
`dropdownMaxHeight` или доступное место, если его меньше. Профильный сценарий один: удалённый поиск по справочнику,
