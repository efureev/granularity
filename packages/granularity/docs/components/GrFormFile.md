...
### Порядок задаёт пользователь

`reorderable` добавляет строке ручку переноса. Нужно там, где последовательность
файлов что-то значит: галерея, страницы документа, очередь вложений. Осмысленно
только с `multiple`.

Механика берётся у [`GrSortableList`](./GrSortableList.md), а не пишется заново:
перенос указателем, клавиатура (`Space` берёт строку, стрелки двигают, `Esc`
отменяет), автопрокрутка и объявления у него уже есть. Цена честная — его стили
приезжают каждому, кто берёт `GrFormFile`.

Тянется **ручка**, а не строка: в строке стоит кнопка удаления, и перетаскивание
всей строки отняло бы у неё клик.

Перестановка отдаёт `update:modelValue` и `change` — как удаление и добавление.
Без второго порядок остался бы только на экране, а в форму ушёл бы прежний.
При `readonly` и `disabled` перенос запрещён.
...
Загрузки на сервер: `GrFormFile` — поле формы, файлы уходят вместе с ней.
Прогресс и отмену закрывает [`GrFileUpload`](./GrFileUpload.md).
