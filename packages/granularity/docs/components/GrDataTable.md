...
### Ограничение: не вместе с `virtual`

Пока раскрытие с виртуализацией не работает и в этом режиме выключается с
предупреждением в dev. Причина в замере: виртуализатор меряет высоту **самой
строки**, а второй ярус — отдельная `<tr>`, и её высота в расчёт не попадает,
отчего распорки разъезжаются молча. Снимется, когда пара строк переедет в свою
группу `<tbody>`.
