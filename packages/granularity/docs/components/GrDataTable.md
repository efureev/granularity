...
- **данные приходят массивом** — колонки объявляются декларативно, разметка строк не пишется;
- **строки сортируют** — по клику на заголовок; `externalSort` передаёт сортировку на сервер;
