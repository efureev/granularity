...
## Клавиатура по ячейкам

`cell-navigation` включает паттерн [`grid`][apg-grid] из WAI-ARIA: таблица
получает `role="grid"`, занимает **одну** остановку `Tab`, и внутрь ведут
стрелки.

| Клавиша | Что делает |
| --- | --- |
| стрелки | по ячейкам: вертикаль по строкам, горизонталь по колонкам |
| `Home` / `End` | к краям **строки**, а не всей таблицы |
| `Enter`, `F2` | войти в содержимое ячейки |
| `Escape` | вернуть фокус самой ячейке |

Шапка — такая же строка сетки: из неё вниз попадаешь в первую строку данных.
Кольцо не замкнуто: строка ниже последней — это не первая, а «дальше некуда».

**Проп отдельный, а не поведение по умолчанию.** Сетка забирает `Tab` себе, и у
всех, кто таблицу уже поставил, таб-порядок бы переставился.

**Контролы внутри ячеек из обхода выходят** — сортировка, ручки переноса и
ширины, меню колонки, чекбокс, кнопка раскрытия. Иначе остановок было бы
столько же, сколько контролов, то есть паттерн не выполнялся бы вовсе: до них
добираются стрелкой до ячейки и `Enter`. Ради этого у [`GrCheckbox`](./GrCheckbox.md)
есть проп `tabindex` — свою остановку он держит литералом, и снаружи её иначе
не снять.

**С виртуализацией работает.** Набор ячеек — отрисованное окно, а не вся
таблица: у десяти тысяч строк на шесть колонок ключей было бы шестьдесят тысяч.
Шаг за край окна прокручивает список и доводит фокус до появившейся строки.
