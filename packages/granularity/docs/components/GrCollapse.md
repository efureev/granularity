...
- **открыта одна секция за раз** — `accordion` закрывает предыдущую;
- **секции нужны незрячему** — `headingLevel` ставит заголовок нужного уровня, и обход по заголовкам работает;
- **открытие надо перехватить** — `beforeChange` откладывает раскрытие до загрузки или подтверждения.

## Когда взять другое

| Нужно | Берите |
| --- | --- |
| Разделы переключаются, а не раскрываются | [`GrTabs`](./GrTabs.md) |
| Разделы формы идут подряд | [`GrFormSection`](./GrFormSection.md) |
| Данные вложены | [`GrTree`](./GrTree.md) |
| Содержимое открывается поверх страницы | [`GrDialog`](./GrDialog.md) / [`GrDrawer`](./GrDrawer.md) |
