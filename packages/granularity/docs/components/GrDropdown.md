...
- **панель открывается по наведению** — `openDelay` и `closeDelay` разводят намерение и случайное движение мыши;
- **открытием управляет родитель** — `v-model:open` вместе с `trigger="manual"`.

## Когда взять другое

| Нужно | Берите |
| --- | --- |
| Пункты обычные: текст, иконка, группы, разделители | [`GrDropdownMenu`](./GrDropdownMenu.md) |
| Внутри форма, фильтр или палитра — не меню | [`GrPopover`](./GrPopover.md) |
| Текстовая подсказка без интерактива | [`GrTooltip`](./GrTooltip.md) |
| Выбор значения поля формы | [`GrSelect`](./GrSelect.md) |
