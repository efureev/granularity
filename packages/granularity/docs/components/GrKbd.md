...
- **сочетание зависит от платформы** — `platform` печатает `⌘` на macOS и `Ctrl` на остальных;
- **клавиш несколько** — `keys` собирает сочетание с разделителем, без ручной вёрстки;
...
| Нужно | Берите |
| --- | --- |
| Показывается статус или ярлык | [`GrBadge`](./GrBadge.md) |
| Нужна подсказка о назначении | [`GrTooltip`](./GrTooltip.md) |
| Показывается фрагмент кода | разметка приложения: блоков кода в пакете нет |
| Сочетание вызывает палитру команд | [`GrCommandPalette`](./GrCommandPalette.md) |
...
## Последовательность пишется в самой строке

Пробел делит `keys` на **шаги**, плюс — клавиши внутри шага:

```vue
<GrKbd keys="g i" />        <!-- G затем I -->

<GrKbd keys="mod+k p" />    <!-- ⌘K затем P -->
```

Так подсказка и привязка описываются **одной строкой**: ту же `'g i'` понимает
`v-hotkey` ([`keyboard.md`](../keyboard.md)). Раньше строка `'g+i'` означала
аккорд для директивы и цепочку для подсказки, а разводил их `variant`, которого
директива не видит.

Граница шага сильнее `variant`: «затем» появляется между шагами независимо от
него. `variant="sequence"` остаётся рабочим для строки без пробелов — старая
запись `keys="g+i" variant="sequence"` даёт прежний результат.

Форма `keys` массивом остаётся плоским аккордом: что там шаг, а что клавиша,
массив не различает.

## Сочетание из реестра

`hotkey` берёт сочетание по id из реестра приложения
(`granularityHotkeysPlugin`) — тогда подсказка не может разойтись с привязкой:

```vue
<GrKbd hotkey="search.open" />
```

Проп сильнее `keys`: заданы оба — побеждает реестр, иначе два источника спорили
бы молча. Неизвестный id в dev-сборке даёт предупреждение, а не пустую подсказку.
