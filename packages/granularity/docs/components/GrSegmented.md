...
| Нужно | Берите |
| --- | --- |
| Вариантов больше пяти | [`GrSelect`](./GrSelect.md) |
| Это значение поля, а не режим | [`GrRadioGroup`](./GrRadioGroup.md) |
| Разделы с разным содержимым | [`GrTabs`](./GrTabs.md) |
| Вариантов два и это «включено/выключено» | [`GrSwitch`](./GrSwitch.md) |
| Действия, а не выбор | [`GrButtonGroup`](./GrButtonGroup.md) |

Сегменты против вкладок: сегменты меняют **вид** одного содержимого, вкладки
переключают **разное** содержимое. Спутать их легко, а пользователю разница
видна сразу — от неё зависит, ждать ли смены всей области.
...
## Кому достаётся свободное место

`block` растягивает ряд на контейнер, а `itemWidth` решает, как он делит ширину:

| Значение | Что делает |
| --- | --- |
| `equal` (умолчание) | сегменты равны — правильно для «День / Неделя / Месяц» |
| `content` | ширина от содержимого, свободное место делится поверх неё |

```vue
<GrSegmented v-model="filter" :options="filters" item-width="content" block />
```

Равные треки в шторке на 400 px дают три ячейки по 121 px: слову «Все» столько
не нужно, а «С документами» не помещается и режется многоточием — при том, что
суммарно места достаточно. `content` раскладывает его туда, где оно нужно.

**Умолчание не меняется, и это осознанно.** С `content` ширина ряда зависит от
длины подписей, и перевод интерфейса переставлял бы границы сегментов у всех
потребителей разом.

Технически режим — это трек `minmax(min-content, auto)`. Минимум `min-content`, а
не `max-content`: там, где места не хватает по-честному, трек обязан сжаться и
отдать подпись многоточию, иначе ряд вылезает за контейнер.
...
Дорожка — пилюля, и это дефолт: так сегменты выглядели всегда. Второе значение —
`box`, то же скругление, что у полей ввода: в форме рядом с `GrSelect` пилюля
читается гостем из другой системы.

```vue
<GrSegmented v-model="view" :options="options" shape="box" />
```

Радиус сегмента внутри дорожки производный (`calc(радиус − поле дорожки)`),
поэтому вторая форма получается сама и отдельной настройки не требует.

В вертикали пилюля считается формулой, а не берёт `9999px`: на высокой колонке
такой радиус превратил бы дорожку в эллипс, поэтому он равен высоте одного
сегмента. `box` этой поправки не знает — 6 px эллипсом не становятся.

Ось общая с полями ввода — [`../sizes.md`](../sizes.md).
...
Подпись сегмента не переносится: ряд обязан оставаться рядом. Не поместившийся
хвост прячется многоточием, и «Включено в подписку» превращается в «Включе…».
