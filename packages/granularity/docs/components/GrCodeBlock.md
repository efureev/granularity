...
Не редактирует, не диффит, не сворачивает узлы и не подсвечивает языки кроме
JSON. Свёртка по узлам с поиском и виртуализацией — отдельная задача, и стоит
она на порядок дороже блока кода.
