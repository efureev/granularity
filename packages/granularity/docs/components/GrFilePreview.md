...
Без `clickable` и `href` это картинка, а не контрол: плитка не занимает
остановку `Tab` и не получает курсор — пустая остановка хуже её отсутствия.
...
**`as` называет тег, а не поведение.** `as="article"` или `as="figure"` меняет
разметку страницы и ничего больше: классы те же, что у `<div>`, курсора и
кольца фокуса нет, родовое имя плитке не навязывается — `link-name` к
`<article>` не применяется. Контролом плитку делают `clickable`, `href` и
компонент-ссылка. Действие, повешенное на тег вне таб-порядка, кликается мышью
и предупреждает в dev-сборке.
