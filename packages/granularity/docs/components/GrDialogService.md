...
- **результат нужен значением** — `confirm`/`prompt` возвращают Promise, и ветвление пишется линейно;
...
| Нужно | Берите |
| --- | --- |
| Окно открывается разметкой и живёт на экране | [`GrDialog`](./GrDialog.md) |
| Подтверждение объявлено в шаблоне | [`GrConfirmDialog`](./GrConfirmDialog.md) |
| Запрос значения объявлен в шаблоне | [`GrPromptDialog`](./GrPromptDialog.md) |
| Сообщение без вопроса | [`GrToaster`](./GrToaster.md) |
| Своя раскладка модального слоя | [`GrModal`](./GrModal.md) |
