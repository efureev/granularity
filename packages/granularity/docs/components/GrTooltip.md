...
`describe-trigger="false"` отменяет `aria-describedby`. Нужно там, где подсказка
дословно повторяет доступное имя триггера: описание тем же текстом диктор
прочитает вторым заходом, и получится «Billing, Billing». Так подключён
свёрнутый `GrSidebarItem` — имя ему даёт `aria-label`, а подсказка нужна
глазам.
...
Обе обёртки — внешняя и триггерная — по умолчанию `inline-flex`, то есть
подсказка сжимается по содержимому. `block` переводит их в `flex w-full`:
нужно, когда триггер обязан занять ширину родителя — пункт списка, строка меню,
ячейка. Проп меняет **обе** обёртки: снаружи до внутренней класс не доходит, и
одной внешней мало — триггер всё равно схлопнулся бы вместе с подсветкой и
кольцом фокуса.
