...
Модель пунктов общая с [`GrDropdownMenu`](./GrDropdownMenu.md), поэтому и
подменю здесь то же самое: пункт с непустыми `children` раскрывает следующий
уровень, `select` по нему не приходит.
...
Клавиатура, коридор для курсора и правила закрытия цепочки описаны там же —
[«Второй уровень: подменю»](./GrDropdownMenu.md#второй-уровень-подменю). Здесь
отличается только корень: выбор в подменю закрывает контекстное меню, а `Escape`
на верхнем уровне возвращает к нижнему, не закрывая всё.
