...
| --- | --- | --- |
| `explorer` | файлы, пространства имён, тысячи узлов | строка 24px и кегль под неё, волосяные нейтральные направляющие, колена связей, подпись ветки без утолщения |
| `rail` | навигация по разделам, боковая панель | строка 30px, выбор — сплошная плашка, скруглённая только справа, с перекрашенным текстом |
| `outline` | оглавление рядом с текстом | ни подложек, ни направляющих: выбор отмечен полосой у края списка и цветом, вес отдан уровню |
| `picker` | отбор узлов отметками | строка 38px и увеличенный квадрат: цель нажатия — сам квадрат, а не подпись |

`picker` отметок не включает — это ортогональная ось (`showCheckbox`), и она
работает в любом виде. Набор задаёт под неё только плотность.

**Семейство шрифта в набор не входит**, а кегль входит: своего токена для
семейства у дерева нет, а высота строки без кегля недостижима — её держит текст,
а не `min-height`. Моноширинный набор проводника подмешивается рядом:
`:style="{ ...grTreeViewVars.explorer, fontFamily: 'var(--gr-font-mono)' }"`.

**Значки и хвост строки — тоже не вид, а слот.** Проводнику значок папки и файла
нужен не для красоты: колено связи упирается в начало содержимого, и без значка
черта обрывается в пустоте перед подписью. Содержимое строки растянуто на всю
свободную ширину, поэтому счётчик или метка прижимаются вправо обычным
`margin-left: auto`.

**Набор кладётся на само дерево, а не на обёртку.** Значения по умолчанию дерево
объявляет на своём корне, поэтому переменная, выставленная предком, до строки не
доходит — её перекрывает собственное объявление. Так же поступает и
`GrTreeSections` со своим видом строк: отдаёт его внутреннему дереву, а не
секции.

`rowClass` перебивает кегль и цвет дерева: обе величины объявлены на корне и
строкой наследуются, а не задаются ей. Ступени по уровням иначе не сделать —
уровень строки знает только разметка.
...
**Свёрнутая ветка под узлом раскрывается сама** — если целиться внутрь неё
(средняя треть строки) и задержаться там. Иначе положить узел внутрь свёрнутой
ветки на нужное место было нельзя вовсе: перенос пришлось бы бросить, раскрыть
её руками и начать заново.

Только на «внутрь»: целясь в соседа сверху или снизу, пользователь не просил
менять раскладку, а раскрытие увело бы строки из-под курсора вместе с точкой, в
которую он метил.

С `virtual` цель — отрисованная строка, и это свойство указателя, а не
недоделка: навестись на то, чего нет на экране, всё равно нечем. У края списка
работает автопрокрутка, и попадание пересчитывается на каждом её кадре —
доехавшая до окна строка становится целью сразу. Клавиатурный перенос от этого
не зависит вовсе: он ходит по модели.
