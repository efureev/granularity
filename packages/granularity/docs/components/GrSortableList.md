...
- **тянуть надо за ручку** — `handleOnly` оставляет текст выделяемым, а строку — кликабельной.

## Когда взять другое

| Нужно | Берите |
| --- | --- |
| Переносить между двумя списками | [`GrTransfer`](./GrTransfer.md) |
| Порядок фиксирован | [`GrList`](./GrList.md) |
| Элементы вложены | [`GrTree`](./GrTree.md) с `draggable` |
| Порядок колонок таблицы | [`GrDataTable`](./GrDataTable.md) |
| Виджеты на двумерной сетке | [`GrDashboard`](../../../granularity-dashboard/docs/components/GrDashboard.md) |
| Строки только выбирают, не двигают | [`GrDataTable`](./GrDataTable.md) |
