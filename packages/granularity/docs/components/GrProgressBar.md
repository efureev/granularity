...
- **конец неизвестен** — `indeterminate` показывает, что процесс идёт, не обещая срока;
- **загружено больше, чем проиграно** — `buffer` рисует второй слой: буферизация видео, предзагрузка;
- **значение надо показать словами** — `showValue` с `formatValue` вместо отдельной подписи.

## Когда взять другое

| Нужно | Берите |
| --- | --- |
| Места по ширине нет | [`GrProgressCircle`](./GrProgressCircle.md) |
| Доля неизвестна и контент уже есть | [`GrLoading`](./GrLoading.md) |
| Контента ещё нет | [`GrSkeleton`](./GrSkeleton.md) |
| Прогресс загрузки файлов | [`GrFileUpload`](./GrFileUpload.md) |
| Показать долю от целого как данные | [`GrChartPie`](../../../granularity-charts/docs/components/GrChartPie.md) |
