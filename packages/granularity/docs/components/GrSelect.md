...
Клавиатура — [`../keyboard.md`](../keyboard.md), слои — [`../z-index.md`](../z-index.md). Форма рамки (`shape`) — [`../sizes.md`](../sizes.md).
...
поле поиска и строки состояний остаются на виду, а полоса прокрутки одна. Механизм общий на списочные
панели пакета и описан в [`../overlays.md`](../overlays.md).

## Виртуализация

`virtual` оставляет в DOM только окно вокруг вьюпорта; его высоту задаёт
`dropdownMaxHeight` — или доступное место, если его меньше. Режим только для `optionsView="panel"` — у нативного
