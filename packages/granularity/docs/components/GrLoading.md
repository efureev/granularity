...
- **ожидание короткое** — `delay` не показывает спиннер вовсе, если ответ пришёл быстро;
- **блокировать надо весь экран** — `fullscreen` на время операции, которую нельзя прерывать;
...
| Нужно | Берите |
| --- | --- |
| Контента ещё нет вовсе | [`GrSkeleton`](./GrSkeleton.md) |
| Известна доля выполненного | [`GrProgressBar`](./GrProgressBar.md) / [`GrProgressCircle`](./GrProgressCircle.md) |
| Ожидание внутри кнопки | [`GrButton`](./GrButton.md) с `loading` |
| Загрузка файла | [`GrFileUpload`](./GrFileUpload.md) |
