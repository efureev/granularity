...
## Две пустоты, а не одна

`kind` различает то, что пользователь видит по-разному:

| `kind` | Когда | Заголовок и иконка по умолчанию |
| --- | --- | --- |
| `empty` (по умолчанию) | ещё ничего не создано | «Пока пусто», ящик |
| `search` | под запросом или фильтром ничего не нашлось | «Ничего не найдено», лупа с крестом |

Разница не косметическая. Под фильтром «Пока пусто» **дезинформирует**: данные
есть, просто не совпали, и следующий шаг у пользователя другой — не «создать», а
«ослабить запрос».

Значений ровно два, потому что столько повторяется. Ошибка загрузки сюда не
входит намеренно: у неё в пакете свои компоненты —
[`GrResponseErrorBanner`](./GrResponseErrorBanner.md).

Описание по умолчанию не подставляется ни в одном из видов: оно всегда про
конкретный экран, и угадать его нечем.
