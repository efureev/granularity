...
уезжает в `aria-describedby` поля: цвет рамки как единственный носитель смысла
— это WCAG 1.4.1, а ARIA-эквивалента `aria-invalid` у этих двух состояний нет.
...
от `GrFormField`, — и признак гаснет, как только поле объявлено невалидным.
 `disabled` гасит поле токенами `--gr-disabled-*`, а не
