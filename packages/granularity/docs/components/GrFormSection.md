...
- **раздел нужен незрячему** — заголовок рендерится тегом `h2`…`h6`, и форма обходится по заголовкам;
...
| Нужно | Берите |
| --- | --- |
| Правила и отправка | [`GrForm`](./GrForm.md) |
| Обвязка одного поля | [`GrFormField`](./GrFormField.md) |
| Разделы переключаются, а не идут подряд | [`GrTabs`](./GrTabs.md) |
| Раздел сворачивается | [`GrCollapse`](./GrCollapse.md) |
| Нужна просто карточка | [`GrCard`](./GrCard.md) |
