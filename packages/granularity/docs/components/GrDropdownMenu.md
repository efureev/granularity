...
Пункт с непустыми `children` становится раскрывателем: он ничего не выполняет и
`select` по нему не приходит — он открывает панель следующего уровня. В
композиции то же самое даёт `GrDropdownMenuSub`.

```ts
const items: GrDropdownMenuEntry[] = [
  { key: 'rename', label: 'Переименовать' },
  { key: 'export', label: 'Экспорт', children: [
    { key: 'pdf', label: 'PDF' },
    { key: 'csv', label: 'CSV' },
  ] },
]
...
```vue
<GrDropdownMenuSub label="Экспорт">
  <GrDropdownMenuItem @click="save('pdf')">PDF</GrDropdownMenuItem>
</GrDropdownMenuSub>
```

Глубина не ограничена, `children` принимает те же записи, что и корень: внутри
подменю осмысленны и разделители, и группы, и следующий уровень.

**Уровни — самостоятельные слои, а не вложенная разметка.** Каждая панель
уезжает в портал, поэтому в DOM они лежат рядом, а не одна в другой. Из этого
следует всё остальное: `Escape` закрывает верхний уровень, а не всё меню;
выбор в подменю закрывает цепочку целиком; клик по раскрывателю до панели
родителя не доходит, иначе `closeOnContentClick` закрыл бы меню ровно в момент
раскрытия следующего уровня.

**Курсору дан коридор.** Путь от пункта к его подменю идёт наискось и проходит
по соседним пунктам. Пока курсор внутри треугольника «точка ухода → ближний край
раскрытой панели», уход считается движением к цели: панель не закрывается, а
соседи своё подменю не раскрывают — они раскроют его, как только станет ясно,
что курсор шёл не к панели. Без этого подменю теряется на полпути.

Клавиатура — [`../keyboard.md`](../keyboard.md): `→` раскрывает, `←` возвращает
на раскрыватель, `Tab` закрывает меню целиком.

`href` и роли-переключатели с `children` несовместимы: пункт не может быть
одновременно ссылкой и раскрывателем — в dev-сборке это предупреждение.
