...
### Значащий аддон объявляется

Аддоны скрыты от скринридера по умолчанию: чаще всего там лупа поиска или глиф,
повторяющий подпись поля, и второе прочтение мешает.

Но `https://` в поле адреса или `₽` при подписи без валюты бывают **единственным
местом**, где сказано, что вводить. Для них есть `describeAddons`.

Он не просто снимает скрытие: `<div>` рядом с полем в его доступное имя не
входит, и диктор наткнулся бы на текст лишь при обходе страницы — не тогда,
когда он нужен. Поэтому аддон уезжает в `aria-describedby` самого поля.

```vue
<GrInput v-model="site" describe-addons>
  <template #prefix>
    https://
  </template>
</GrInput>
...
`shape="pill"` даёт то же скругление, что у `GrSegmented`; дефолт — `box`,
скругление шкалы контролов.

Форма тянет за собой горизонтальный отступ: у пилюли он не меньше половины
высоты, иначе текст заезжает в дугу. Аддоны идут за ней же — отсек обрезается по
внешнему контуру оболочкой (`overflow-hidden`), а его разделительная рамка
остаётся прямой.

Ось и её границы — [`../sizes.md`](../sizes.md); там же сказано, каким полям
формы не досталось и почему.
...
`success` и `warning` вдобавок показывают иконку и скрытую подпись, которая
уезжает в `aria-describedby` поля. Цвет рамки — единственный носитель смысла —
это WCAG 1.4.1: иконка добавляет форму, подпись добавляет программный канал,
которого у этих двух состояний нет (ARIA-эквивалента `aria-invalid` для них не
существует). `danger` иконки не получает: его смысл уже несут `aria-invalid` и
текст ошибки от `GrFormField`.

Признак гаснет, когда поле объявлено невалидным: галочка «корректно» рядом с
`aria-invalid="true"` противоречила бы ему.
