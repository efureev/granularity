...
Набор совпадает с [`GrInput`](./GrInput.md) (кроме `clear` — кнопки очистки у
текстовой области нет), чтобы обёртки над контролами писались одинаково.
Нативные события переизлучаются компонентом: объявленный emit уходит из
`$attrs`, и без этого `@change` у потребителя перестал бы работать.
