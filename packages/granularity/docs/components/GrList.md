...
**`as` называет тег, а не поведение.** Строка, которой тег нужен ради разметки,
остаётся ровно тем же, чем была:

```vue
<GrListItem title="Профиль" as="article" />
```

Меняется только тег: ни подсветки под курсором, ни кольца фокуса, ни атрибута
`data-gr-list-item-action`. Контролом строку делают `href`, `clickable` и
компонент-ссылка — то есть поведение, которое кто-то действительно обещал.

`as="li"` тут не подходит, и это не придирка: контейнер списка — `div` с
`role="list"`, а `<li>` обязан лежать в `<ul>` или `<ol>` (axe: `listitem`).
Роль пункта уже несёт обёртка — тег из `as` нужен содержимому, а не списку.

**Действие на теге вне таб-порядка — ошибка вёрстки.** `<span>` с обработчиком
клика работает для мыши и только для неё: он не попадает в таб-порядок и не
отвечает на Enter (WCAG 2.1.1). Клик такая строка эмитит — молча выключить его
значило бы сломать работающий код, — но кольца фокуса не получает и ловит
предупреждение в dev-сборке. Компоненты роутера проходят молча: они рендерят
`<a>`, и до рендера этого не узнать.
