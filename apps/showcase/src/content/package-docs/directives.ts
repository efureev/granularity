    ...
    examples: createReadyExamples([
      {
        ...
        code: [
          ...
          '',
          '// `exclude` ждёт элементы. В шаблоне ref разворачивается сам, а в `<script>`',
          '// геттер обязан вернуть `excludeEl.value`: Vue-ref директива отбросит молча.',
          'const excludeEl = ref<HTMLElement | null>(null)',
          '',
          '<button ref="excludeEl">Exclude zone</button>',
          '',
          '<div',
          '  v-if="open"',
          '  v-click-outside="{',
