  ...
  GrImageCropSize,
        ...
        examples: [
          ...
          },
          {
            ...
            title: 'How the avatar looks everywhere',
            description: 'Живой предпросмотр в четырёх местах сразу: шапка, строка списка, комментарий, крупная карточка.',
            previewKey: 'extra-media-image-crop-avatar',
            note: 'Главное сомнение при кадрировании — «а как это будет смотреться маленьким»: лицо, занимающее весь круг на превью в 300 px, в строке списка превращается в пятно. Ответ даёт только показ рядом, в настоящих размерах. Предпросмотр обновляется с задержкой в четверть секунды: `crop()` рисует холст и кодирует файл, и делать это на каждый пиксель перетаскивания значит греть процессор ради кадров, которых никто не увидит.',
          },
          {
