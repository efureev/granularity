...
import type { DataSourceUrlAdapter } from '@feugene/granularity-datasource/url'`,
  ...
  GrCodeScannerSize,
        ...
        examples: [
          ...
          {
            id: 'media-code-scanner-receiving',
            title: 'Goods-in at the warehouse',
            description: 'Коробки сканируют подряд: повтор увеличивает количество, новый код добавляет строку.',
            previewKey: 'extra-media-code-scanner-receiving',
            note: 'Здесь видно, зачем нужен `continuous`: без него второй такой же штрихкод не дал бы события вовсе, и кладовщик решил бы, что сканер не сработал. С ним повтор — законная вторая единица товара. Три кода опознаются по справочнику, остальные попадают как «Неизвестная позиция»: на складе это отдельная задача приёмщика, а не ошибка сканера.',
          },
        ],
      },
      {
        ...
        overview: {
          ...
          features: [
            ...
            'Кадр снимается целиком и в пропорциях камеры: подгонять его под окно бессмысленно — на разных устройствах камеры отдают разное.',
  ...
  GrCameraStatus,
        ...
        apiSections: [
          {
            ...
            items: [
              ...
              { name: 'autoStart', type: 'boolean', default: 'false', description: 'Включать камеру сразу. По умолчанию нет — запрос без действия пользователя отклоняют не глядя.' },
              { name: 'aspectRatio', type: 'number', description: 'Пожелание камере: какое соотношение запросить у устройства. Не обрезка — камера вправе отдать своё, и показан будет её настоящий кадр.' },
              { name: 'mirror', type: 'boolean', description: 'Зеркалить превью. Без него зеркалится только фронтальная камера.' },
              { name: 'output', type: '{ width?, height?, type?, quality? }', description: 'Габарит и кодек снимка: кадр вписывается в заданные размеры, сохраняя пропорции съёмки. По умолчанию JPEG с качеством 0.92.' },
              { name: 'size', type: `'xs' | 'sm' | 'md' | 'lg'`, description: 'Шкала вложенных кнопок; читается из `GrConfigProvider`.' },
        ...
        examples: [
          ...
          {
            id: 'media-camera-to-crop',
            title: 'Shoot, crop, send',
            description: 'Путь целиком: снимок с камеры уезжает в кадрирование, оттуда — готовый аватар.',
            previewKey: 'extra-media-camera-to-crop',
            note: 'Камера снимает кадр целиком, в своих пропорциях: она не знает, что из него понадобится, а на разных устройствах отдаёт разное. Вырезает нужное следующий шаг — `GrImageCrop`. Разделение выглядит придиркой на словах и очевидным на экране: один компонент производит файл, другой его сужает.',
          },
        ],
      },
      {
  ...
  GrImageCropSize,
        ...
        apiSections: [
          {
            ...
            items: [
              ...
              { name: 'output', type: '{ width?, height?, type?, quality? }', description: 'Габарит и кодек результата: кадр вписывается в заданные размеры, сохраняя пропорции области. Без `output` — в пикселях исходника.' },
        ...
        examples: [
          ...
          {
            id: 'media-image-crop-avatar',
            title: 'One frame, three files',
            description: 'Из одного кадра собираются три варианта — крупный, средний и мелкий, — и каждый показан там, где он потом появится.',
            previewKey: 'extra-media-image-crop-avatar',
            note: 'Приложения хранят аватар не одной картинкой: в шапку идёт крупный, в строку списка — мелкий, и отдавать 256 px туда, где рисуется 24, значит возить лишние килобайты на каждой строке. Размер задаёт `output`, кадр остаётся тем же. Заодно закрывается главное сомнение при кадрировании — «а как это будет смотреться маленьким»: в кружке 24 px сразу заметно, что в кадр попало лишнее. Пересборка идёт с задержкой: три `crop()` подряд рисуют холст и кодируют файл, и делать это на каждый пиксель перетаскивания значит греть процессор ради кадров, которых никто не увидит.',
          },
          {
            id: 'media-image-crop-weight',
            title: 'Weight against quality',
            description: 'Формат и качество меняются на живом кадре, рядом — вес получившегося файла.',
            previewKey: 'extra-media-image-crop-weight',
            note: 'Разницу между webp и jpeg описанием пропа не объяснишь: это два числа, и увидеть их можно только рядом. Заодно видно, что png качество игнорирует вовсе — он сжимает без потерь, поэтому ползунок для него выключен. Практический вывод: фотографиям webp с качеством около 0.8, скриншотам с текстом — png.',
          },
        ],
      },
    ],
  },
  {
    ...
    description: 'Размеченный текст: поле на TipTap со схемой вместо санитайзера и показ markdown деревом узлов вместо `v-html`.',
    dependencies: ['@tiptap/core', '@tiptap/pm', '@tiptap/starter-kit', '@tiptap/extensions', 'marked'],
    components: [
      {
        name: 'GrMarkdown',
        slug: 'gr-markdown',
        title: 'GrMarkdown',
        summary: 'Показ markdown: описание товара, README в админке, комментарий, ответ языковой модели. Дерево узлов вместо строки HTML.',
        importPath: '@feugene/granularity-editor/components/GrMarkdown',
        overview: {
          paragraphs: [
            'Показывает текст, сохранённый в markdown: описание товара, README в админке, комментарий в тикете, ответ языковой модели. В отличие от `GrRichText`, где размеченный документ правят, здесь его только читают — и компонент не несёт ни редактора, ни его веса.',
            'Дерево собирается узлами Vue, а не строкой HTML: экранирование делает сам Vue, поэтому санитайзера в пакете нет, а `v-html` не появится. Сырой HTML показывается текстом либо выбрасывается — третьего значения у пропа нет.',
            'Разбирает CommonMark и GFM плюс сноски и алерты GitHub. Длинный документ листается без виртуализации, правка одного абзаца стоит одного абзаца, а поток от языковой модели не дорожает по мере ответа.',
          ],
          lists: [
            {
              title: 'Когда брать',
              items: [
                'Пользовательский текст с разметкой: описание товара от поставщика, комментарий в тикете, заметка в CRM — источник недоверенный, и рендер без строки HTML снимает целый класс уязвимостей.',
                'Документация внутри продукта: README репозитория, справка, заметки о релизе, changelog — с якорями у заголовков и оглавлением из `parsed`.',
                'Ответ языковой модели: `streaming` делает цену куска независимой от длины уже написанного, а незакрытая ограда не мигает.',
                'Живой предпросмотр рядом с полем ввода markdown — там, где пишут текст и сразу смотрят на результат.',
                'Письма и уведомления, тело которых хранится markdown и показывается и в интерфейсе, и в рассылке.',
              ],
            },
            {
              title: 'Что каким компонентом рисуется',
              items: [
                '`- [ ]` и `- [x]` — `GrCheckbox`: состояние задачи в `modelValue`, `readonly` пока не включён `interactiveTasks`.',
                '`> [!NOTE]` — `GrAlert` с `tone="info"`, `[!TIP]` — `success`, `[!IMPORTANT]` — `primary`, `[!WARNING]` — `warning`, `[!CAUTION]` — `danger`.',
                'Маркер алерта разбирается только в тон: заливку, рамку и иконку задаёт `GrConfigProvider`, своего `variant` компонент не навязывает.',
                'Заголовок тона берётся из локали пакета — тон обязан читаться текстом, а не только цветом; живого региона у сообщения нет (`live="off"`).',
                'Таблица GFM — `GrTable`: скролл-область с именем из локали, шапка и строки идут слотами.',
                'Заголовок, абзац, список, цитата, ссылка, разделитель и блок кода — семантическая разметка на токенах `--gr-markdown-*`, без рёбер к ядру.',
              ],
            },
            {
              title: 'Границы ответственности',
              items: [
                'Не редактор: компонент показывает и сам источник не меняет. Правка форматированного текста — `GrRichText`, правка markdown как кода — `GrCodeEditor`.',
                'Не рендерит HTML: `<div>` внутри markdown останется текстом. Кому нужен HTML — подменяет рендерер узла и берёт ответственность на себя.',
                'Не подсвечивает код сам: блок нарисован со своей рамкой, ярлыком языка и копированием, но не раскрашен. Подсветка приходит пропом `components.code`.',
                'Не знает математику и диаграммы: `$…$` и блок `mermaid` остаются текстом — обе вещи тянут сотни килобайт и живут в точке расширения.',
                'Не хранит и не отправляет: источник даёт приложение, оно же решает, что делать с `taskToggle` и `linkClick`.',
              ],
            },
          ],
          features: [
            'CommonMark и GFM: таблицы, задачи, зачёркивание, автоссылки. Плюс сноски и алерты GitHub (`> [!NOTE]`).',
            'Правка абзаца стоит абзаца: блок, текст которого не изменился, отдаёт тот же VNode, и Vue пропускает его целиком.',
            'Длинный документ листается без виртуализации — раскладка снимается `content-visibility`, а поиск по странице, якоря и печать продолжают работать.',
            'Стриминг: цена куска не зависит от того, сколько уже написано — разбирается только хвост.',
            'Ссылка с чужой схемой становится текстом: проверка идёт после развёртывания сущностей, а не наивным `startsWith`.',
            'Любой узел подменяется своим рендерером: блок кода на подсвечивающий, ссылка на роутерную, картинка на просмотрщик.',
          ],
        },
        typeDeclarations: `import type {
  GrMarkdownProps,
        ...
        examples: [
          {
            ...
            description: 'Заголовки с якорями, списки, задачи, таблица, код, цитата, алерты и сноска. Конструктор меняет то, что решают один раз на приложение.',
            previewKey: 'extra-editor-markdown-basic',
            note: 'Мера строки ограничивает только текстовые блоки, а таблица и код выходят на всю ширину: иначе таблица упиралась бы в колонку текста. Якорь заголовка есть в DOM всегда и лишь показывается по наведению — появляющийся элемент с клавиатуры недостижим. `id` заголовка считается из его текста, а не через `useId()`: иначе ссылка вида `#раздел` из письма перестала бы работать.',
          },
          {
            id: 'markdown-source',
            title: 'Слева источник, справа результат',
            description: 'Правят markdown слева — справа он пересобирается сразу. Односторонняя связь: источник остаётся единственной правдой.',
            previewKey: 'extra-editor-markdown-source',
            note: 'Счётчик блоков справа показывает, во что превратился текст. Правка одного абзаца перерисовывает один блок — остальные Vue пропускает целиком, потому что их VNode не изменились.',
          },
          {
            id: 'markdown-stream',
            title: 'Ответ приходит потоком',
            description: 'Текст набегает кусками, как ответ языковой модели. Счётчик рядом читает DOM и показывает, сколько блоков сменилось на последнем куске.',
            previewKey: 'extra-editor-markdown-stream',
            note: 'Число держится на единице при любой длине уже написанного: пустая строка закрывает блок, и всё выше уходит в стабильный префикс — перелексируется только хвост. Незакрытая ограда сразу рисуется блоком кода, а не тремя кавычками, иначе на каждом куске моргала бы вся вёрстка.',
          },
          {
            id: 'markdown-renderers',
            title: 'Подмена рендерера узла',
            description: 'Тот же документ с подменой рендереров: подсветка кода из `granularity-code`, ссылки во внутренний роутер, своя картинка.',
            previewKey: 'extra-editor-markdown-renderers',
            note: '`GrMarkdown` про `granularity-code` не знает — подсвечивающий блок приносит потребитель, и ребра между пакетами не заводится. Последний тумблер про другое: он даёт движку токенайзер. `==текст==` — это выделение маркером из Obsidian и Pandoc, синтаксиса нет ни в CommonMark, ни в GFM, и без расширения парсер его не узнаёт — на экране остаются сами знаки равно. Токен приезжает узлом `custom` и ищет компонент в `components` по своему имени; не нашёл — печатает исходник текстом, а не теряет его.',
          },
          {
            id: 'markdown-mermaid',
            title: 'Диаграммы: пакет ставит приложение',
            description: 'Ограда с языком `mermaid` превращается в схему. Движок приносит потребитель — в пакете его нет ни в зависимостях, ни в коде.',
            previewKey: 'extra-editor-markdown-mermaid',
            note: 'Шов тот же, что у подсветки, — `components.code`. Импорт динамический: документ без диаграмм за движок не платит, а выключенный тумблер не загружает вовсе ничего. Две строки конфига здесь не косметика: `layout: \'dagre\'` отсекает раскладку ELK, которая иначе приезжает на первый же flowchart и весит 440 КБ gzip — больше, чем всё остальное вместе; `securityLevel: \'strict\'` вычищает разметку из подписей, потому что markdown пришёл извне. SVG вставляется разобранными узлами, а не `v-html`: директивы, которой нет во всём репозитории, нет и здесь.',
          },
        ],
        apiSections: [
          {
            ...
            origin: 'manual',
            items: [
              { name: 'source', type: 'string', description: 'Текст в markdown.' },
              { name: 'preset', type: `'gfm' | 'commonmark' | 'minimal'`, default: `'gfm'`, description: '`gfm` — таблицы, задачи, зачёркивание; `commonmark` — без них; `minimal` ещё и превращает картинку в её подпись.' },
              { name: 'html', type: `'escape' | 'strip'`, default: `'escape'`, description: 'Что делать с сырым HTML. Рендера в разметку нет ни при каком значении — он потребовал бы `v-html`.' },
              { name: 'headingOffset', type: 'number', default: '0', description: 'Сдвиг уровней заголовков: документ внутри страницы, где `h1` уже занят. За `h6` не выходит.' },
              { name: 'anchors / idPrefix', type: 'boolean / string', description: 'Ссылка-якорь у заголовка и префикс `id`. Префикс нужен, когда документов на странице несколько: слаг считается из текста и потому одинаков.' },
              { name: 'measure / density / size', type: `boolean / 'comfortable' | 'compact' / 'xs' | 'sm' | 'md' | 'lg'`, description: 'Мера строки, плотность и кегль. `density` и `size` настраиваются глобально через `GrConfigProvider`.' },
              { name: 'linkTarget / linkRel / allowedProtocols', type: 'string | null / string / string[]', description: 'Политика ссылок. Схема вне списка отбрасывается — ссылка становится текстом.' },
              { name: 'interactiveTasks', type: 'boolean', default: 'false', description: 'Чекбоксы задач кликабельны. Эмит несёт офсет маркера в исходнике, а не номер пункта.' },
              { name: 'streaming', type: 'boolean', default: 'false', description: 'Источник дописывается в конец: разбирается только хвост.' },
              { name: 'components', type: 'GrMarkdownRenderers', description: 'Подмена рендерера узла: `code`, `link`, `image`, `heading`, `table`.' },
              { name: 'engine', type: 'GrMarkdownEngine', description: 'Свой движок разбора вместо `marked`. Интерфейс не упоминает ни одного его типа.' },
              { name: 'ariaLabel', type: 'string', description: 'Имя области. Не задан — ориентиром документ не объявляется: иначе каждый комментарий в ленте стал бы им.' },
            ],
          },
          {
            key: 'events',
            ...
            origin: 'manual',
            items: [
              { name: 'parsed', type: '(headings: GrMdHeading[], blocks: number) => void', description: 'Оглавление в форме `GrScrollSpySection` и число блоков.' },
              { name: 'taskToggle', type: '(offset: number, checked: boolean) => void', description: 'Офсет маркера `[ ]` в исходнике: по нему приложение переписывает markdown.' },
              { name: 'linkClick', type: '(event: MouseEvent, href: string | null) => void', description: 'Клик по ссылке — чтобы увести во внутренний роутер.' },
              { name: 'copy / error', type: '(language: string | null) / (error: unknown) => void', description: 'Копирование блока кода и отказ подменённого движка.' },
            ],
          },
          {
            key: 'expose',
            ...
            origin: 'manual',
            items: [
              { name: 'headings', type: 'ComputedRef<GrMdHeading[]>', description: 'Оглавление документа — им кормят `GrScrollSpy` ядра.' },
              { name: 'scrollToHeading', type: '(id: string) => void', description: 'Прокрутка к разделу. Под `prefers-reduced-motion` — мгновенная.' },
              { name: 'plainText', type: '() => string', description: 'Текст без разметки: поиск, превью в списке, доступное имя.' },
            ],
          },
        ],
      },
  ...
  GrRichTextSize,
        ...
        examples: [
          ...
          {
            id: 'editor-frame',
            title: 'Header and footer inside the field',
            description: 'Слоты `#header` и `#footer` дают полосы над текстом и под ним — внутри той же рамки, что и поле.',
            previewKey: 'extra-editor-frame',
            note: 'Снаружи подпись и счётчик читались отдельным блоком: рамка обводила только текст, и связь с полем держалась на близости. Линия принадлежит границе между зонами, а не самой зоне — у края поля она сошлась бы со скруглением рамки. Фона у зон нет: подложка `--gr-muted` есть у тулбара, потому что он панель управления, а тут содержимое потребителя.',
          },
          {
            id: 'editor-toolbar',
          ...
          {
            id: 'editor-to-markdown',
            title: 'From the editor back to markdown',
            description: 'Правят в `GrRichText`, наружу уходит markdown или родной JSON редактора. Ниже — тот же markdown, показанный `GrMarkdown`.',
            previewKey: 'extra-editor-markdown-from-editor',
            note: 'Обратную дорогу парсер не закрывает: он работает в одну сторону, и markdown из документа собирает отдельная функция `markdownFromEditorDocument` из слоя `./markdown`. Она ходит по обычному JSON и `@tiptap/*` не импортирует, поэтому проверяется без поднятия ProseMirror. Круг замыкается целиком: документ, затем markdown, затем дерево, затем разметка.',
          },
          {
            id: 'editor-schema',
