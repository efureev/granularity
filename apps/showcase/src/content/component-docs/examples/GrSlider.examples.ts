...
export const grSliderExamples: ShowcaseComponentExampleDoc[] = [
  {
    ...
    status: 'ready',
    previewKey: 'gr-slider-basic',    note: 'Каждый бегунок — `role="slider"` с `aria-valuemin`/`max`/`now`, доступный с клавиатуры и для скринридеров.',
  },
  {
    id: 'slider-range',
    ...
    status: 'ready',
    previewKey: 'gr-slider-range',    note: 'Для диапазона у нижнего бегунка `aria-valuemax` = значение верхнего, а у верхнего `aria-valuemin` = значение нижнего — скринридер объявляет корректные границы.',
  },
  {
    id: 'slider-marks',
  ...
  },
  {
    ...
    status: 'ready',
    previewKey: 'gr-slider-custom',    note: 'Переменные наследуются, поэтому одну тему слайдеров можно задать на контейнере формы, а отдельные слайдеры точечно переопределить.',
  },
]
