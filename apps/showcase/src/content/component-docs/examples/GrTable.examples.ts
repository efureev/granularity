...
export const grTableExamples: ShowcaseComponentExampleDoc[] = [
  ...
  {
    id: 'table-sticky-column',
    title: 'Липкая первая колонка',
    description: '`sticky-column` оставляет первую колонку на месте при горизонтальной прокрутке: иначе у широкой таблицы видно числа, но не видно, чьи они. Вместе со `sticky-header` держатся обе оси.',
    status: 'ready',
    previewKey: 'gr-table-sticky-column',
    note: 'Только первая колонка, и это не упрощение: `left` со второй равен сумме ширин предыдущих, а ячейки принадлежат потребителю — мерить их «тонкому» контейнеру нечем. Многоколоночное закрепление есть у `GrDataTable`, который знает свои колонки. Фон липкой ячейки наследуется от строки, поэтому полосатость и подсветка работают в ней сами.',
  },
]
