...
export const dsModalExamples: ShowcaseComponentExampleDoc[] = [
  {
    id: 'modal-basic-flow',
    title: 'Bare modal flow',
    description: 'Базовый сценарий для `DsModal`: минимальный контейнер, открытие по кнопке и явное закрытие из пользовательского контента.',
    status: 'ready',
    previewKey: 'ds-modal-basic-flow',
    code: `<script setup lang="ts">
import { ref } from 'vue'

import { DsButton, DsModal } from '@feugene/granularity'
...
<template>
  <div class="flex flex-wrap gap-3">
    <DsButton variant="outline" @click="openWithSize('sm')">
      Open compact modal
    </DsButton>
    <DsButton @click="openWithSize('lg')">
      Open wide modal
    </DsButton>
  </div>

  <DsModal v-model="open" :size="activeSize">
