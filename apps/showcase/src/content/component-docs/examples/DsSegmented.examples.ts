...
export const dsSegmentedExamples: ShowcaseComponentExampleDoc[] = [
  {
    id: 'segmented-basic-pills',
    title: 'Pills variant for compact view switching',
    description: 'Базовый happy-path для `DsSegmented`: лёгкий pills-control с moving indicator и выбором одного значения.',
    status: 'ready',
...
<template>
  <DsSegmented v-model="period" :options="options" aria-label="Period" />
</template>`,
  },
  {
...
<template>
  <DsSegmented v-model="view" :options="options" variant="button" aria-label="View" />
</template>`,
  },
  {
...
<template>
  <DsSegmented v-model="value" :options="options" aria-label="View" />
</template>`,
  },
  {
    id: 'segmented-states',
...
<template>
  <div class="grid gap-4">
    <DsSegmented v-model="locale" :options="localeOptions" size="sm" aria-label="Language" />
    <DsSegmented v-model="status" :options="statusOptions" block variant="button" aria-label="Publishing status" />
  </div>
</template>`,
  },
]