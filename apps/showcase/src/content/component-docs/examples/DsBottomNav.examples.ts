...
export const dsBottomNavExamples: ShowcaseComponentExampleDoc[] = [
  {
...
import { DsBottomNav } from '@feugene/granularity'
...
<template>
  ...
  },
  {
    ...
    description: 'Показываем, что `v-model` у `DsBottomNav` можно менять не только через сам компонент, но и из внешних action-кнопок.',
    status: 'ready',
    previewKey: 'ds-bottom-nav-external-state',
    code: `<script setup lang="ts">
import { ref } from 'vue'

import { DsBottomNav, DsButton } from '@feugene/granularity'
...
<template>
  <div class="flex flex-wrap gap-2">
    <DsButton size="sm" variant="outline" @click="currentSection = 'tasks'">
      Jump to tasks
    </DsButton>
  </div>
