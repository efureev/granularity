...
export const dsFormFileExamples: ShowcaseComponentExampleDoc[] = [
  {
...
<template>
  ...
  },
  {
    ...
    description: 'Отдельно фиксируем `validate`/`update:errors`: showcase должен показать, что `DsFormFile` подходит и для domain-specific upload rules, а не только для `accept`.',
    status: 'ready',
    previewKey: 'ds-form-file-custom-validation',
    code: `<script setup lang="ts">
import { ref } from 'vue'

import { DsBadge, DsFormField, DsFormFile } from '@feugene/granularity'
