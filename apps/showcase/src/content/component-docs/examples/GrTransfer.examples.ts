...
export const grTransferExamples: ShowcaseComponentExampleDoc[] = [
  ...
  },
  {
    ...
    description: 'Правая панель переставляется перетаскиванием и `Alt` + стрелкой, и её порядок уезжает в `v-model`. Именно этим компонент отличается от мультиселекта, где выбранное сжато в строку и порядка не имеет.',
    status: 'ready',
