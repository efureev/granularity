...
export const grSidebarExamples: ShowcaseComponentExampleDoc[] = [
  ...
  {
    id: 'sidebar-overlay',
    title: 'Модальный слой на узком экране',
    description: '`overlay` превращает панель в модальный слой: подложка, `Esc`, ловушка фокуса, блокировка прокрутки — всё из общего `useModalOverlay`, которым живут `GrDrawer` и `GrPopover`. Когда включать, решает приложение: своей системы брейкпоинтов у пакета нет, а спрашивать среду в `setup` нельзя — первый клиентский рендер разошёлся бы с серверным. Разметка панели одна на оба режима.',
    status: 'ready',
    previewKey: 'gr-sidebar-overlay',
  },
  {
    id: 'sidebar-nested',
