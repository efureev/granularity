...
const size = ref<DsSwitchSize>('md')
...
const sizeOptions = [
  ...
  {value: 'md', label: 'MD'},
  {value: 'lg', label: 'LG'},
] satisfies Array<{ value: DsSwitchSize, label: string }>
...
const previewCode = computed(() => {
  ...
  return ['<DsSwitch', ...attributes.map(attribute => `  ${attribute}`), '>', `  ${switchText.value}`, '</DsSwitch>'].join('\n')
})
</script>
  ...
  <div class="grid gap-4 xl:grid-cols-[minmax(0,1.15fr)_320px]">
    ...
    <div class="showcase-demo-panel grid gap-4 rounded-[28px] border p-4 lg:p-5">
      ...
      <div class="grid gap-4">
        ...
        <DsFormField label="Active background color">
          <DsInput
        ...
        <DsFormField label="Inactive background color">
          <DsInput
      ...
      <div class="grid gap-3 rounded-2xl border border-[var(--brd)] bg-[var(--card)] p-4">
        <DsSwitch v-model="checked" size="sm">
          Checked
        </DsSwitch>
