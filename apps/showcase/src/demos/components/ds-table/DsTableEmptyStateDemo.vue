...
<template>
  <div class="grid gap-3">
    <div>
      <DsButton size="sm" variant="outline" @click="empty = !empty">
      ...
      <tr v-if="empty" class="border-t border-[var(--brd)]">
        <td colspan="3" class="px-4 py-6">
          <DsEmptyState
            ...
            description="Для data-display сценариев `DsTable` может хостить empty state прямо внутри tbody без дополнительного shell-компонента."
          >
            <DsButton size="sm" @click="empty = false">
              Load sample data
            </DsButton>
          </DsEmptyState>
        </td>
