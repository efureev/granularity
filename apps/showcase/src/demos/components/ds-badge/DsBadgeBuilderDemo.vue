...
const tone = ref<DsBadgeTone>('primary')
const size = ref<DsBadgeSize>('md')
const radius = ref<DsBadgeRadius>('round')
...
] satisfies Array<{ value: DsBadgeTone, label: string }>

const sizeOptions = [
  {value: 'sm', label: 'SM'},
  {value: 'md', label: 'MD'},
  {value: 'lg', label: 'LG'},
] satisfies Array<{ value: DsBadgeSize, label: string }>

const radiusOptions = [
  {value: 'square', label: 'Square'},
  {value: 'semi', label: 'Semi'},
  {value: 'round', label: 'Round'},
] satisfies Array<{ value: DsBadgeRadius, label: string }>
...
const previewCode = computed(() => {
  ...
  return ['<DsBadge', ...attributes.map(attribute => `  ${attribute}`), '>', `  ${content}`, '</DsBadge>'].join('\n')
})
</script>
      ...
      <div
        ...
        <div class="flex h-full flex-col items-center justify-center gap-5 text-center">
          ...
          <div class="flex flex-wrap items-center justify-center gap-3">
            <DsBadge :tone="tone" :size="size" :radius="radius" :dark="dark">
    ...
    <div class="showcase-demo-panel grid gap-4 rounded-[28px] border p-4 lg:p-5">
      ...
      <div class="grid gap-4">
        <DsFormField label="Tone">
          <DsSelect v-model="tone" :options="toneOptions" aria-label="Badge tone"/>
        </DsFormField>
        ...
        <DsFormField label="Radius">
          <DsRadioGroup v-model="radius" :options="radiusOptions" variant="button" size="sm"/>
        </DsFormField>

        <DsFormField label="Label">
          <DsInput v-model="label" placeholder="Beta" aria-label="Badge label"/>
        </DsFormField>
      </div>

      <div class="grid gap-3 rounded-2xl border border-[var(--brd)] bg-[var(--card)] p-4">
        <DsSwitch v-model="dark" size="sm">
          Filled / dark mode
        </DsSwitch>
        <DsSwitch v-model="uppercase" size="sm">
          Uppercase label
        </DsSwitch>
      </div>
    </div>
