...
export const showcasePages: ShowcasePage[] = [
  ...
  },
  {
    ...
    eyebrow: 'Showcase / Architecture',
    status: 'ready',
    topNavigation: false,
    description: 'Граф зависимостей между компонентами: что ни от чего не зависит, на ком держится библиотека и сколько компонентов приедет вместе с выбранным при гранулярной селекции.',
    sections: [
      {
        id: 'base',
        title: 'Базовые компоненты',
        description: 'Компоненты, которые не рендерят внутри себя ни одного соседа: любой из них можно взять в приложение поодиночке.',
        bullets: [
          'Источник правды — `config.dependencies` каждого компонента, тот же, что читают гейты пакета.',
          'Базовый компонент при гранулярной селекции приносит ровно свой CSS и safelist.',
        ],
      },
      {
        id: 'load-bearing',
        title: 'На ком держится',
        description: 'Рейтинг по числу компонентов, которые рендерят этот компонент внутри себя.',
        bullets: [
          'Правка опорного компонента затрагивает всех, кто его рендерит.',
          'Число зависимых — это и оценка риска регрессии, и подсказка, где нужен визуальный гейт.',
        ],
      },
      {
        id: 'levels',
      ...
      {
        id: 'focus',
        title: 'Окрестность компонента',
        description: 'Что компонент тянет вниз и кто использует его самого.',
        bullets: [
          'Цена гранулярной селекции: `components: [X]` приносит весь путь вниз целиком.',
          'Полотно из всех компонентов сразу нечитаемо, поэтому показывается окрестность одного.',
        ],
      },
      {
        id: 'soft-links',
        title: 'Связи, которые не рёбра',
        description: 'Контекст и композаблы связывают компоненты сильнее, чем граф рендера, но чужую разметку не тянут.',
        bullets: [
          '`GrConfigProvider` даёт дефолты размеров, `GrFormField` — контекст поля, `shared/` — шкалы и тона.',
          'Объявить их зависимостями значит притащить каждому потребителю весь CSS и safelist донора.',
        ],
      },
    ],
  },
    ...
    sections: [
      {
        ...
        bullets: [
          ...
          '`@feugene/granularity-charts` — графики на своём SVG, `@feugene/granularity-chrono` — календарь и пикеры без сторонних виджетов.',
