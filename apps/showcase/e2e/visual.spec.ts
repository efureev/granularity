  ...
  test.describe(`visual overlay (${theme})`, () => {
    for (const target of VISUAL_OVERLAYS) {
      test(`${target.name} panel`, async ({ page }) => {
        ...
        /*
         * Хром витрины гасится и здесь, а не только у снимков страниц.
         *
         * Панель `GrImageViewer` полноэкранна, и сквозь её подложку в кадр
         * попадает шапка сайта — а в шапке бейдж версии ядра. Замерено: ширина
         * «v0.49.0» и «v0.50.0» отличается на доли пикселя, соседняя навигация —
         * `flex-1`, её левый край уезжает субпиксельно, и каждый глиф строки
         * перерастрируется. Отклонение 2 уровня из 255 — картинки неразличимы
         * глазом, — но пикселей набирается 4251 при бюджете 300, и эталон стоял
         * красным с выпуска 0.50.0.
         *
         * Маской это не лечится, и здесь её быть не должно: прямоугольник
         * Playwright рисуется поверх страницы, и у панели, доходящей до верха
         * экрана, он ложится на её собственную шапку — замерено на `GrDrawer`,
         * где розовый бокс накрыл заголовок ящика.
         */
        await hideChrome(page)
