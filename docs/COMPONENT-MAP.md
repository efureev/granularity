...
**Планируется в ядро.** Вложенные подменю — второго уровня пунктов нет ни у
`GrDropdownMenu`, ни у `GrContextMenu`. Это доработка их обоих, а не отдельный
компонент.

Отдельного `GrMenu` в планах нет: всплывающие меню закрывают `GrContextMenu`
(растёт из точки курсора) и `GrDropdownMenu` (висит на кнопке), а вертикальное
меню навигации — это `GrSidebar` с `landmark="navigation"`, группами и
вложенными пунктами.
